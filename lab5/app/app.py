    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if user_role == 'Администратор':
                ...
                # Пользователь может редактировать и смотреть только себя
                if action in ['view_user', 'edit_user']:
                    target_user_id = kwargs.get('user_id')
                    if target_user_id == current_user.id:
                        has_access = True
                elif action == 'view_logs':
                    has_access = True # Фильтрация записей происходит внутри самого view
...
@app.before_request
def start_timer():
    # Просто запоминаем путь. Flask-Login еще не прогрузил юзера.
    if request.endpoint != 'static':
        g.visit_path = request.path

@app.after_request
def log_visit(response):
    # Эта функция срабатывает КОГДА ЮЗЕР УЖЕ ОПРЕДЕЛЕН, но страница еще не улетела в браузер
    path = getattr(g, 'visit_path', None)
    if path:
        try:
            # Здесь current_user уже работает правильно!
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        if not errors:
            ...
            return redirect(url_for('users'))
        flash('Допущены ошибки при заполнении формы', 'danger')
    return render_template('edit_user.html', title='Редактирование', user=user, roles=roles, errors=errors, current_user=current_user)
